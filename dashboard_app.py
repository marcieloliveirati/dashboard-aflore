...
# ==========================================
        ...
        <style>
            ...
            @media print {
                * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
                html, body, [class*="stApp"], [data-testid="stAppViewContainer"], .main { background-color: #ffffff !important; background-image: none !important; }
                p, span, div, h1, h2, h3, h4, h5, h6, text, label, li { color: #000000 !important; }
                [data-baseweb="select"] > div, [data-testid="stSelectbox"] div { background-color: #ffffff !important; border-color: #cccccc !important; }
                ...
                [data-testid="metric-container"], .gps-box { background-color: #f8f9fa !important; border-left: 5px solid #00d48a !important; border-radius: 8px !important; padding: 10px !important; margin-bottom: 10px !important; page-break-inside: avoid !important; break-inside: avoid !important; }
                div[data-testid="stMetricValue"] > div { font-size: 1.2rem !important; }
                [data-testid="stPlotlyChart"], [data-testid="stPlotlyChart"] > div, .js-plotly-plot, .js-plotly-plot .main-svg { background-color: transparent !important; background: transparent !important; page-break-inside: avoid !important; break-inside: avoid !important; }
                ...
                .page-break { display: block !important; page-break-before: always !important; break-before: page !important; height: 0 !important; margin: 0 !important; padding: 0 !important; }
                [data-testid="stDataFrame"] { display: none !important; } 
                .print-only { display: block !important; page-break-inside: avoid !important; }
                .tabela-pdf { width: 100%; border-collapse: collapse; font-family: 'Helvetica Neue', sans-serif; font-size: 0.85rem; margin-top: 10px; background-color: #ffffff !important; color: #000000 !important; }
                .tabela-pdf th, .tabela-pdf td { border: 1px solid #e0e0e0 !important; padding: 8px !important; text-align: left !important; background-color: #ffffff !important; color: #000000 !important; }
                .tabela-pdf th { background-color: #f8f9fa !important; color: #00d48a !important; font-weight: bold !important; }
                section[data-testid="stSidebar"], header[data-testid="stHeader"], .btn-imprimir, .stButton { display: none !important; }
    ...
    if os.path.exists("logo_aflore.png"):
        ...
        with col2: st.image("logo_aflore.png", use_column_width=True)
    elif os.path.exists("logo.png"):
        col1, col2, col3 = st.sidebar.columns([1, 2, 1])
    ...
    # --- CABEÇALHO DINÂMICO PRINCIPAL ---
    col_titulo, col_imprimir, col_sair = st.columns([7, 2, 1])
    
    with col_titulo:
        st.title("📊 Vision Sale - Inteligência de Vendas")
        # Alterna o subtítulo dinamicamente para não empilhar textos repetidos
        if modulo_selecionado == "📈 Visão Geral de Lojas":
            st.subheader("Painel Analítico de Desempenho de Lojas")
        else:
            st.subheader("🔥 Inteligência Promocional - Tabloides")
        
    with col_imprimir:
        st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
        if st.button("🖨️ Gerar PDF", use_container_width=True):
            components.html(f"<script>setTimeout(function() {{ window.parent.print(); }}, 1000);</script><span style='display:none'>{time.time()}</span>", height=0, width=0)
        ...
        def parse_data(val):
            ...
            try:
                ...
                if len(partes) >= 2:
                    ...
                    if mes_str in meses_map: mes = meses_map[mes_str]
                    else: mes = int(''.join(filter(str.isdigit, partes[1])))
                        
                    ano = date.today().year
                    if len(partes) >= 3:
                        ano_str = ''.join(filter(str.isdigit, partes[2]))
                        if len(ano_str) == 4: ano = int(ano_str)
                        elif len(ano_str) == 2: ano = 2000 + int(ano_str)
        ...
        try:
            ...
            if not url_planilha:
                ...
                if xls_dict:
                    ...
                    # Inteligência de Seleção de Aba Vigente
                    ...
                    if idx_cabecalho >= 0:
                            ...
                            for num_loja, col_vl in enumerate(vl_indices):
                                ...
                                try:
                                    ...
                                    if vl > 0 or clientes > 0:
                                        all_data.append({
                                            'Data_Raw': val_data_crua, 'Loja': nome_loja, 'Venda_Liquida': vl, 
                                            'Ticket_Medio': tmv, 'PA': pa, 'Clientes': int(clientes),
                                            'Meta_Faturamento': metas_dinamicas[nome_loja]['vl'], 'Meta_Clientes': metas_dinamicas[nome_loja]['cli'], 'Meta_TMV': metas_dinamicas[nome_loja]['tmv']
                                        })
                        ...
                        if not df.empty:
                            ...
                            # Conversão blindada para garantir compatibilidade nos filtros
                    ...
                    if df.empty: st.sidebar.warning(f"⚠️ Aba: {aba_selecionada} sem dados válidos.")
                    else: st.sidebar.success(f"✅ Aba '{aba_selecionada}' lida com sucesso!")
            ...
            if df_filtrado.empty:
                ...
                if pd.notna(max_data_planilha):
                    ...
                    if mes_fechado:
                        ...
                        if dias_uteis_restantes > 0: nova_meta_diaria = meta_restante / dias_uteis_restantes
                        else: nova_meta_diaria = 0 if meta_restante <= 0 else meta_restante
                ...
                if mes_fechado:
                    ...
                    if faturado_mes_total >= meta_mes_total: cor_alerta, mensagem_status = "#00d48a", "✅ Meta superada!"
                    else: cor_alerta, mensagem_status = "#f72585", "⚠️ Meta não atingida."
                    ...
                    <div class="gps-box" style='background-color: rgba(128,128,128, 0.1); padding: 20px; border-radius: 10px; border-left: 8px solid {cor_alerta}; margin-bottom: 25px; margin-top: 15px;'>
                        ...
                        <div style='display: flex; justify-content: space-between; flex-wrap: wrap;'>
                            <div style='flex: 1; min-width: 200px;'><p class="gps-label" style='margin:0; font-size: 0.9rem; color: #888888;'>Meta Total Original</p><p class="gps-val" style='margin:0; font-size: 1.4rem; font-weight: bold;'>{str_meta_mes}</p></div>
                            <div style='flex: 1; min-width: 200px;'><p class="gps-label" style='margin:0; font-size: 0.9rem; color: #888888;'>Faturamento Consolidado</p><p style='margin:0; font-size: 1.4rem; font-weight: bold; color: {cor_alerta};'>{str_fat_mes}</p></div>
                            <div style='flex: 1; min-width: 200px;'><p class="gps-label" style='margin:0; font-size: 0.9rem; color: #888888;'>Status de Performance</p><p class="gps-val" style='margin:0; font-size: 1.1rem;'><b>{mensagem_status}</b></p></div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    ...
                    <div class="gps-box" style='background-color: rgba(128,128,128, 0.1); padding: 20px; border-radius: 10px; border-left: 8px solid {cor_alerta}; margin-bottom: 25px; margin-top: 15px;'>
                        ...
                        <div style='display: flex; justify-content: space-between; flex-wrap: wrap;'>
                            <div style='flex: 1; min-width: 200px;'><p class="gps-label" style='margin:0; font-size: 0.9rem; color: #888888;'>Meta Diária Original</p><p class="gps-val" style='margin:0; font-size: 1.4rem; font-weight: bold;'>{str_meta_orig}</p></div>
                            <div style='flex: 1; min-width: 200px;'><p class="gps-label" style='margin:0; font-size: 0.9rem; color: #888888;'>Meta Diária Exigida (Hoje)</p><p style='margin:0; font-size: 1.4rem; font-weight: bold; color: {cor_alerta};'>{str_nova_meta}</p></div>
                            <div style='flex: 1; min-width: 200px;'><p class="gps-label" style='margin:0; font-size: 0.9rem; color: #888888;'>Status do Mês</p><p class="gps-val" style='margin:0; font-size: 1.1rem;'>Restam <b>{dias_uteis_restantes} dias úteis</b>. {mensagem_status}</p></div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                with col_dir:
                    ...
                    fig_barra = px.bar(df_barras, x='Loja', y='Valor', color='Tipo', barmode='group', 
                                       ...
                                       labels={'Valor': 'Faturamento (R$)', 'Tipo': 'Métrica'}, category_orders={'Tipo': ['Previsto', 'Realizado']})
                ...
                df_plot_acumulado = df_diario.rename(columns={'Realizado_Acumulado': 'Realizado Acumulado', 'Meta_Acumulada': 'Meta Projetada Acumulada'})
                ...
                html_top, html_bottom = df_top10.to_html(index=False, classes="tabela-pdf"), df_bottom10.to_html(index=False, classes="tabela-pdf")
                html_final_pdf = f"""
                <div class="print-only" style="page-break-before: always;">
                    ...
                    <div style="display: flex; gap: 20px;">
                        <div style="flex: 1; min-width: 0;"><h4 style="color: #00d48a; margin-bottom: 10px; font-size: 1rem;">🏆 Dias de Maior Faturamento (Picos)</h4>{html_top}</div>
                        <div style="flex: 1; min-width: 0;"><h4 style="color: #f72585; margin-bottom: 10px; font-size: 1rem;">⚠️ Dias de Menor Faturamento (Vales)</h4>{html_bottom}</div>
                    </div>
            ...
            with st.spinner("Conectando ao banco de dados de tabloides..."):
                try:
                    ...
                    def limpar_numerico(coluna, is_percentage=False):
                        ...
                        if is_percentage: return valores / 100.0
                        return valores
                    ...
                    total_fat_atual, total_fat_mes_ant, total_fat_ano_ant = df_filtrado['FAT_ATUAL'].sum(), df_filtrado['FAT_MES_ANT'].sum(), df_filtrado['FAT_ANO_ANT'].sum()
                    total_qtd_atual, total_qtd_mes_ant, total_qtd_ano_ant = df_filtrado['QTD_ATUAL'].sum(), df_filtrado['QTD_MES_ANT'].sum(), df_filtrado['QTD_ANO_ANT'].sum()
                    ...
                    with col_g2:
                        ...
                        if not top_produtos.empty:
                            top_produtos['🔥 Cresc. YoY (%)'] = top_produtos['Crescimento_Bruto'].apply(lambda x: f"{x:,.2f}%".replace(',', 'X').replace('.', ',').replace('X', '.'))
                            st.dataframe(top_produtos[['SKU', 'DESCRIÇÃO', '🔥 Cresc. YoY (%)']], hide_index=True, use_container_width=True)
                        else:
                    ...
                    colunas_exibicao = ['LOJA', 'SKU', 'DESCRIÇÃO', 'CATEGORIA', 'PREÇO "POR"', 'QTD VENDIDA (Ano Anterior)', 'QTD VENDIDA (Mês Anterior)', 'QTD VENDIDA (Tabloide Atual)', 'CRES. VOLUME YoY', 'CRES. VOLUME MoM', 'VENDA BRUTA (Ano Anterior)', 'VENDA BRUTA (Mês Anterior)', 'VENDA BRUTA (Tabloide Atual)']
